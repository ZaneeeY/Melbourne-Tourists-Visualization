  ...
  selected_restaurant_data <- reactive({
    ...
    # If a specific cuisine is chosen, filter the rows accordingly
     ...
     # if(input$selectedCuisine == "All"){
      ...
      #}
    #}
    if (!is.null(input$tableauViz_mark_selection_changed$all_splited_cuisines[1])) {
      filtered_data <- filtered_data[grepl(input$tableauViz_mark_selection_changed$all_splited_cuisines[1], filtered_data$cuisines), ]
    }else{
      filtered_data <- restaurant_data
    }
